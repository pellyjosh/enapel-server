...
The server is packaged as a NativePHP (Electron) desktop app. The Windows `.exe` is generated by `electron-builder`
and bundles the Laravel app plus a portable PHP runtime.
...
It prepares a clean build `.env`, applies the NativePHP vendor patch, and then builds the installer.
...
- If startup fails, check `%LOCALAPPDATA%\\Enapel Server\\logs\\startup.log` on the Windows host.
