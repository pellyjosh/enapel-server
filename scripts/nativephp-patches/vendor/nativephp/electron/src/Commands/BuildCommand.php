...
class BuildCommand extends Command
{
    ...
    private function updateElectronDependencies(): void
    {
        ...
        Process::path(__DIR__.'/../../resources/js/')
            ...
            ->forever()
            ->run('npm ci && npm run plugin:build', function (string $type, string $output) {
                echo $output;
            });
    }
    ...
    private function buildOrPublish(): void
    {
        ...
        // Ensure PHP binary is extracted BEFORE electron-builder starts packing,
        ...
        foreach ($commandsToRun as $archArgs) {
            ...
            Process::path(__DIR__.'/../../resources/js/')
                ->env($this->getEnvironmentVariables())
                ->forever()
                ->run("node php.js {$archArgs}", function (string $type, string $output) {
                    echo $output;
                });
        }
